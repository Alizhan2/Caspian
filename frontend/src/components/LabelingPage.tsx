...
function WeatherContext({ sample, language }: { sample: LabelSample; language: Language }) {
  const weather = sample.weather_context;
  const labels = language === "ru"
    ? { title: "Ветер во время съёмки", speed: "Скорость", direction: "Направление", gusts: "Порывы", unavailable: "Исторические данные пока недоступны" }
    : { title: "Түсірілім кезіндегі жел", speed: "Жылдамдық", direction: "Бағыт", gusts: "Екпін", unavailable: "Тарихи деректер әзірге қолжетімсіз" };
  if (!weather || weather.status !== "available") return <div className="weather-context unavailable"><strong>{labels.title}</strong><span>{labels.unavailable}</span></div>;
  return <div className="weather-context"><strong>{labels.title}</strong><span>{labels.speed}: <b>{weather.wind_speed_10m_ms?.toFixed(1)} m/s</b></span><span>{labels.direction}: <b>{weather.wind_direction_10m_deg?.toFixed(0)}°</b></span><span>{labels.gusts}: <b>{weather.wind_gusts_10m_ms == null ? "—" : `${weather.wind_gusts_10m_ms.toFixed(1)} m/s`}</b></span></div>;
}

function TemporalContext({ sample, language }: { sample: LabelSample; language: Language }) {
  const context = sample.temporal_context;
  const labels = language === "ru"
    ? { title: "Сравнение с прошлой сценой", days: "Интервал", changed: "Изменившиеся пиксели", unavailable: "Нет более ранней сравнимой сцены", note: "Контрастный screening-индикатор, не подтверждение разлива." }
    : { title: "Алдыңғы көрініспен салыстыру", days: "Аралық", changed: "Өзгерген пиксельдер", unavailable: "Бұрынғы салыстырмалы көрініс жоқ", note: "Контрастқа негізделген screening-индикаторы, төгіндіні растамайды." };
  if (!context || context.status !== "available") return <div className="temporal-context unavailable"><strong>{labels.title}</strong><span>{labels.unavailable}</span></div>;
  return <div className="temporal-context"><strong>{labels.title}</strong><span>{labels.days}: <b>{context.days_between?.toFixed(1)} {language === "ru" ? "дн." : "күн"}</b></span><span>{labels.changed}: <b>{((context.changed_pixel_fraction ?? 0) * 100).toFixed(1)}%</b></span><small>{labels.note}</small></div>;
}
